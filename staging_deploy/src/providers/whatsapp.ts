...
export class WhatsAppClient {
    ...
    private readonly storeFile = path.join(process.cwd(), 'data', 'baileys_store.json');
    ...
    constructor() {
        ...
        process.on('unhandledRejection', (reason, promise) => {
            try { console.error('[CRITICAL] Unhandled Rejection:', reason); } catch (e) { /* ignore */ }
        });
        let exceptionBusy = false;
        process.on('uncaughtException', (err: any) => {
            if (exceptionBusy || (err && err.code === 'EPIPE')) return;
            exceptionBusy = true;
            try {
                console.error('[CRITICAL] Uncaught Exception thrown:', err);
                this.addLog('CRITICAL', `Uncaught Exception: ${err.message || err}`);
            } catch (e) { /* ignore */ }
            setTimeout(() => { exceptionBusy = false; }, 100);
        });
    ...
    private async loadContactsFromDb() {
        try {
            const rows: any[] = await dbQuery(`SELECT jid, name, notify FROM contacts`);
            let count = 0;
            for (const row of rows) {
                if (row.jid) {
                    this.contacts.set(row.jid, { name: row.name || undefined, notify: row.notify || undefined });
                    count++;
                }
            }
            this.addLog('INFO', `[Store] Contatos carregados do banco: ${count}`);
        } catch (e: any) {
            this.addLog('WARNING', `Erro ao carregar contatos: ${e.message}`);
        }
    }
    ...
    private async syncContactToDb(contact: { id: string; name?: string; verifiedName?: string; notify?: string }) {
        if (!contact.id) return;
        // Deduplicate by JID (phone number). INSERT OR REPLACE keeps the latest name.
        const name = contact.name || contact.verifiedName || null;
        const notify = contact.notify || null;
        try {
            await dbRun(
                `INSERT INTO contacts (jid, name, notify, last_seen)
                 VALUES (?, ?, ?, datetime('now'))
                 ON CONFLICT(jid) DO UPDATE SET
                     name    = COALESCE(excluded.name, contacts.name),
                     notify  = COALESCE(excluded.notify, contacts.notify),
                     last_seen = datetime('now')`,
                [contact.id, name, notify]
            );
        } catch (e: any) {
            // Non-critical: log and continue
            this.addLog('WARNING', `syncContactToDb failed for ${contact.id}: ${e.message}`);
        }
    }
    ...
    private async checkUpcomingReminders() {
        ...
        try {
            ...
            for (const event of events) {
                ...
                const localStatus = event.local_status;
                const isHandled = summary.includes('[CONCLUÍDO]') || summary.includes('[CONCLUÍDA]') || 
                                 summary.includes('[CANCELADO]') || summary.includes('[CANCELADA]') || 
                                 summary.includes('[ADIADO]')    || summary.includes('[ADIADA]') ||
                                 localStatus === 'done' || localStatus === 'canceled' || localStatus === 'postponed';
    ...
    public async sendEventConfirmation(event: any, isUpdate = false) {
        ...
        // 1. SUPRESSÃO DE NOTIFICAÇÃO: Se o evento já estiver concluído/cancelado, 
        // não enviamos mensagem de "confirmar presença" ou "nova reunião".
        // Fazemos isso ANTES de verificar se está offline para não poluir a fila de mensagens.
        if (summary.includes('[CONCLUÍDO]') || summary.includes('[CONCLUÍDA]') || 
            summary.includes('[CANCELADO]') || summary.includes('[CANCELADA]') || 
            summary.includes('[ADIADO]')    || summary.includes('[ADIADA]') ||
            localStatus === 'done' || localStatus === 'canceled' || localStatus === 'postponed') {
            // Logamos apenas se for um log relevante ou se necessário para debug silencioso
            return;
        }
    ...
    public async resetConnection(): Promise<void> {
        ...
        if (fs.existsSync(this.storeFile)) fs.unlinkSync(this.storeFile);
    ...
    public async connect(): Promise<void> {
        ...
        // Sync contacts to in-memory Map AND SQLite
        ...
        this.sock.ev.on('contacts.update', (updates) => {
            for (const update of updates) {
                ...
                this.syncContactToDb(update as any);
            }
        });
        ...
        this.sock.ev.on('messaging-history.set', ({ contacts, chats }) => {
            ...
            // Also extract contacts from chat data (names from pushName etc)
            if (chats) {
                for (const chat of chats) {
                    if (!chat.id || chat.id.endsWith('@g.us') || chat.id.endsWith('@newsletter')) continue;
                    const existing = this.contacts.get(chat.id) || {};
                    if (chat.name && !existing.name) {
                        this.contacts.set(chat.id, { name: chat.name, notify: existing.notify });
                    } else if (chat.id) { // Added this block to handle cases where chat.name might be missing but chat.id is present
                        this.contacts.set(chat.id, { name: chat.id, notify: existing.notify });
                    }
                }
                this.addLog('INFO', `Total de contatos após history sync: ${this.contacts.size}`);
            }
        });
    }
    ...
    private handleConnectionUpdate(update: Partial<ConnectionState>): void {
        ...
        if (connection === 'close') {
            ...
            if (isLoggedOut) {
                ...
                const statusCode = (error as any)?.output?.statusCode || (error as any)?.status || 0;
                this.addLog('INFO', `WhatsApp: conexão interrompida (${statusCode}). Tentando reestabelecer...`);
                
                let delay = 10000;
                if (statusCode === 440) {
                    this.addLog('WARNING', 'Erro 440 (Conflict/Stream Error) detectado. O WhatsApp encerrou a sessão forçadamente.');
                    delay = 5000; // Retry faster or slower? Baileys usually suggests a slight delay.
                }
            ...
            // If contacts map is empty, wait for messaging-history.set events
            setTimeout(async () => {
                if (this.contacts.size === 0 && this.sock) {
                    try {
                        this.addLog('INFO', 'Contacts still empty after 5s, trying chats fetch...');
                        const chats = await (this.sock as any).groupFetchAllParticipating?.();
                        if (chats) {
                            this.addLog('INFO', `Chats fetch returned ${Object.keys(chats).length} groups.`);
                        }
                    } catch (e: any) {
                        this.addLog('WARNING', `Chat fetch failed: ${e.message}`);
                    }
                } else {
                    this.addLog('INFO', `Contacts OK: ${this.contacts.size} contatos na memória.`);
                }
            }, 5000);
        }
    }
        ...
        for (const msg of messages) {
            ...
            if (!msg.key.fromMe && msg.message && msg.key.remoteJid !== 'status@broadcast') {
                ...
                if (text) {
                    ...
                    if (cmd === '#' || cmd.startsWith('#agenda') || cmd.startsWith('#hoje') || cmd.startsWith('#semana') || cmd.startsWith('#atividades') || cmd.startsWith('#reunioes')) {
    ...
    private async handleCalendarCommands(cmd: string): Promise<string> {
        try {
            ...
            if (cmd.includes('#atividades')) {
                typeFilter = 'atividade';
                timeMax = now + (1000 * 60 * 60 * 24 * 30);
                periodName = 'Suas Atividades';
            } else if (cmd.includes('#reunioes')) {
                typeFilter = 'reuniao';
                timeMax = now + (1000 * 60 * 60 * 24 * 30);
                periodName = 'Suas Reuniões';
            ...
            const filteredEvents = events.filter(event => {
                ...
                // 2. Filtro de Status (Ignorar concluídos/cancelados por padrão nas listagens)
                const summary = (event.summary || '').toUpperCase();
                const localStatus = event.local_status;
                const isHandled = summary.includes('[CONCLUÍDO]') || summary.includes('[CONCLUÍDA]') || 
                                 summary.includes('[CANCELADO]') || summary.includes('[CANCELADA]') || 
            ...
            if (filteredEvents.length === 0) {
                return `📅 Você não tem itens pendentes em: *${periodName}*.`;
            }
            ...
            for (const event of filteredEvents) {
                ...
                const type = this.extractTag(event, 'EVENT_TYPE') === 'atividade' ? '📝' : '👥';
    ...
    public async sendMessage(to: string, text: string): Promise<void> {
        ...
        try {
            ...
            this.addLog('SUCCESS', `Mensagem enviada com sucesso para ${to}`);
        } catch (error: any) {
            this.addLog('ERROR', `Erro ao enviar mensagem para ${to}: ${error.message}`);
            throw error;
        }
    }
    ...
    public async offerCall(to: string): Promise<string> {
        ...
        try {
            // Garantir JID individual limpo (sem sufixo de dispositivo)
            let cleanJid = to.split(':')[0];
            if (!cleanJid.includes('@')) {
                cleanJid = `${cleanJid.replace(/\D/g, '')}@s.whatsapp.net`;
            }

            // O Baileys não tem offerCall direto — usa createCallLink para criar link de chamada
            // e depois envia como mensagem ou usa o link para iniciar a chamada
            this.addLog('INFO', `📞 Criando link de chamada para ${cleanJid}...`);

            const token = await this.sock.createCallLink('audio');
            
            if (!token) {
                throw new Error('Não foi possível criar o link de chamada.');
            }

            const callUrl = `https://call.whatsapp.com/voice/${token}`;
            this.addLog('INFO', `🔗 Link de chamada gerado: ${callUrl}`);

            // Enviar o link como mensagem para o contato iniciar a chamada
            await this.sock.sendMessage(cleanJid, {
                text: `📞 *Chamada de Voz WhatsApp*\n\nClique no link para entrar na chamada:\n${callUrl}`
            });

            this.addLog('SUCCESS', `✅ Link de chamada enviado para ${cleanJid}`);
            return token;
        } catch (error: any) {
            this.addLog('ERROR', `❌ Falha ao iniciar chamada para ${to}: ${error.message}`);
            throw error;
        }
    }
    ...
    public async getContacts(): Promise<{ id: string, name?: string, notify?: string }[]> {
        try {
            // PRIMARY: use SQLite (2000+ contacts persisted across restarts)
            const rows = await dbQuery('SELECT jid, name, notify FROM contacts ORDER BY name ASC');
            if (rows.length > 0) {
                // Merge with in-memory map (in-memory may have more recent names)
                return rows
                    .filter((r: any) => r.jid && (r.jid.endsWith('@s.whatsapp.net') || r.jid.endsWith('@lid')))
                    .map((r: any) => {
                        const mem = this.contacts.get(r.jid);
                        return {
                            id: r.jid,
                            name: mem?.name || r.name,
                            notify: mem?.notify || r.notify
                        };
                    });
            }
        } catch (e: any) {
            this.addLog('WARNING', `SQLite contact fetch failed: ${e.message}`);
        }

        // FALLBACK: in-memory only
        if (this.contacts.size === 0) {
            this.addLog('INFO', 'getContacts: lista ainda vazia após SQLite e memória.');
        }
