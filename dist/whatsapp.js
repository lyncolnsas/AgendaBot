...
class WhatsAppClient {
    googleCalendar = new google_calendar_1.GoogleCalendarService();
    sock = null;
    lastQr = null;
    connectionStatus = 'initializing';
    notifiedCallEvents = new Set();
    notifiedReminders = new Set();
    deletedGroups = new Set();
    contacts = new Map();
    logsPath = path_1.default.join(process.cwd(), 'system.log');
    notificationFile = path_1.default.join(process.cwd(), 'notification.json');
    contactsCacheFile = path_1.default.join(process.cwd(), 'data', 'contacts_cache.json');
    messageQueueFile = path_1.default.join(process.cwd(), 'data', 'message_queue.json');
    messageQueue = [];
    ...
    loadContactsCache() {
        try {
            if (fs_1.default.existsSync(this.contactsCacheFile)) {
                const data = JSON.parse(fs_1.default.readFileSync(this.contactsCacheFile, 'utf8'));
    ...
    loadMessageQueue() {
        try {
            ...
            if (fs_1.default.existsSync(this.messageQueueFile)) {
                this.messageQueue = JSON.parse(fs_1.default.readFileSync(this.messageQueueFile, 'utf8')) || [];
    ...
    saveMessageQueue() {
        try {
            ...
            if (!fs_1.default.existsSync(dir))
                fs_1.default.mkdirSync(dir, { recursive: true });
            fs_1.default.writeFileSync(this.messageQueueFile, JSON.stringify(this.messageQueue, null, 2));
        }
        catch (e) {
    ...
    queueSaveContacts() {
        if (this.saveContactsTimer)
            clearTimeout(this.saveContactsTimer);
        this.saveContactsTimer = setTimeout(() => {
            try {
                const data = Object.fromEntries(this.contacts);
                if (!fs_1.default.existsSync(path_1.default.dirname(this.contactsCacheFile))) {
                    fs_1.default.mkdirSync(path_1.default.dirname(this.contactsCacheFile), { recursive: true });
                }
                fs_1.default.writeFileSync(this.contactsCacheFile, JSON.stringify(data), 'utf8');
            }
            catch (e) {
    ...
    MAX_LOG_FILES = 3; // keep system.log.1, .2, .3
    addLog(level, message) {
        const entry = `[${new Date().toISOString()}] [${level}] ${message}\n`;
        // Rotate log if it exceeds max size
        try {
            if (fs_1.default.existsSync(this.logsPath)) {
                const size = fs_1.default.statSync(this.logsPath).size;
                if (size >= this.MAX_LOG_SIZE) {
                    ...
                    for (let i = this.MAX_LOG_FILES; i > 1; i--) {
                        ...
                        if (fs_1.default.existsSync(older))
                            fs_1.default.renameSync(older, newer);
                    }
                    fs_1.default.renameSync(this.logsPath, `${this.logsPath}.1`);
                }
            }
        }
        catch (e) {
        ...
        fs_1.default.appendFileSync(this.logsPath, entry);
    ...
    async getNotificationNumber() {
        try {
            if (fs_1.default.existsSync(this.notificationFile)) {
                const data = JSON.parse(fs_1.default.readFileSync(this.notificationFile, 'utf8'));
    ...
    async sendDailySummary() {
        ...
        if (!target || this.connectionStatus !== 'open')
            return;
    ...
    async checkUpcomingReminders() {
        ...
        try {
            const now = new Date();
            const lookahead = (0, date_fns_1.addMinutes)(now, 60);
            const events = await this.googleCalendar.getEvents(now.getTime(), lookahead.getTime());
            for (const event of events) {
                const startStr = event.start?.dateTime;
                if (!startStr)
                    continue;
                const startDate = (0, date_fns_1.parseISO)(startStr);
                ...
                if (diffMinutes >= 14 && diffMinutes <= 16) {
                    ...
                    if (!this.notifiedReminders.has(reminderKey)) {
                        ...
                        const hourStr = (0, date_fns_1.format)(startDate, 'HH:mm');
            ...
            if (this.notifiedCallEvents.size > 1000)
                this.notifiedCallEvents.clear();
            ...
            for (const event of pastEvents) {
                ...
                if (!endStr)
                    continue;
                const endDate = (0, date_fns_1.parseISO)(endStr);
                ...
                if (diffAfterEnd >= 60 && diffAfterEnd <= 90) {
                    ...
                    if (groupJid && !this.deletedGroups.has(groupJid)) {
                        ...
                        try {
                            ...
                            const members = metadata.participants.map((p) => p.id).filter((id) => id !== this.sock.user.id);
                            if (members.length > 0) {
                                await this.sock.groupParticipantsUpdate(groupJid, members, 'remove');
                            }
            ...
            try {
                ...
                if (url.startsWith('/uploads/')) {
                    ...
                    if (fs_1.default.existsSync(localPath)) {
                        fileBuffer = fs_1.default.readFileSync(localPath);
                ...
                if (isImage) {
                    ...
                    const mimeMap = { pdf: 'application/pdf', zip: 'application/zip', docx: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' };
    ...
    async sendEventConfirmation(event, isUpdate = false) {
        ...
        if (startStr) {
            const dt = (0, date_fns_1.parseISO)(startStr);
            dateStr = (0, date_fns_1.format)(dt, 'dd/MM/yyyy');
            hourStr = (0, date_fns_1.format)(dt, 'HH:mm');
        }
        const callMinutes = this.extractTag(event, 'CALL');
        let callNotice = '';
        if (callMinutes) {
        ...
        if (groupMode === 'create' && !groupJid) {
            ...
            try {
                const group = await this.sock.groupCreate(groupName, members);
    ...
    async sendEventCancellation(event) {
        if (!this.sock || this.connectionStatus !== 'open') {
        ...
        if (groupJid) {
            try {
                ...
                await this.sock.sendMessage(groupJid, { text: cancelMsg });
                await new Promise(r => setTimeout(r, 2000));
                ...
                try {
                    await this.sock.chatModify({ delete: true, lastMessages: [{ key: { remoteJid: groupJid, fromMe: true }, messageTimestamp: Date.now() }] }, groupJid);
                }
                catch (e) {
    ...
    async resetConnection() {
        ...
        if (this.sock) {
            try {
                this.sock.logout('Vazando por reset do usuário');
            }
            catch { }
            this.sock = null;
        }
        const authPath = path_1.default.join(process.cwd(), 'auth_info_baileys');
        if (fs_1.default.existsSync(authPath))
            fs_1.default.rmSync(authPath, { recursive: true, force: true });
        if (fs_1.default.existsSync(this.contactsCacheFile))
            fs_1.default.unlinkSync(this.contactsCacheFile);
        this.contacts.clear();
    ...
    handleConnectionUpdate(update) {
        ...
        if (connection === 'close') {
            ...
            const isLoggedOut = statusCode === baileys_1.DisconnectReason.loggedOut; // 401
            // ── Status code guide ────────────────────────────────────────
    ...
    async handleIncomingCall(calls) {
    ...
    async handleCalendarCommands(cmd) {
        try {
            const now = Date.now();
            let timeMax;
            let periodName;
            ...
            for (const event of events) {
                ...
                if (!start) {
                    ...
                    const dt = (0, date_fns_1.parseISO)(start);
                    const formatted = (0, date_fns_1.format)(dt, 'dd/MM HH:mm');
                    sb += `🔹 *${summary}*\n   🕒 ${formatted}\n\n`;
                }
            }
            return sb;
    ...
    // Public API Methods
    async getUserInfo() {
        if (!this.sock || this.connectionStatus !== 'open' || !this.sock.user)
            return null;
        try {
    ...
    async getQrData() {
        ...
        try {
            const qrImage = await qrcode_1.default.toDataURL(this.lastQr);
    ...
    async offerCall(to) {
        ...
        const callId = await this.sock.offerCall(to);
